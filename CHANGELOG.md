...
- Register migration paths during provider boot so `migrate:run` sees the queue
  operations schema through the same CLI lifecycle used by other extension migrations.
